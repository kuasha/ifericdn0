...
controllers.controller('RolesCtrl', ['$scope', '$modal', 'CosmosService', function ($scope, $modal, CosmosService) {
    ...
    $scope.addRole = function (size, current_role) {
        ...
        modalInstance.result.then(function (role) {
            ...
            if (!role._id) {
                CosmosService.post('/ios/service/cosmos.rbac.object.role/', role, function (data) {
                        console.log(data);
                        $scope.getRoles();
                    },
                ...
                delete role._id;
                delete role.sid;
                CosmosService.put('/ios/service/cosmos.rbac.object.role/' + role_id + '/', role, function (data) {
                        $scope.getRoles();
                    },
                    function (data, status) {
                        $scope.processError(data, status);
    ...
    $scope.removeRole = function (roleIndex) {
        ...
        if (confirm('Are you sure you want to delete the role ' + role.name + '?')) {
            var role_id = role._id;
            CosmosService.delete('/ios/service/cosmos.rbac.object.role/' + role_id + '/', function (data) {
                    $scope.getRoles();
                },
                function (data, status) {
                    $scope.processError(data, status);
