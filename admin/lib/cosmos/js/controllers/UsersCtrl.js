...
controllers.controller('UsersCtrl', ['$scope', '$modal', 'CosmosService', function ($scope, $modal, CosmosService) {
    ...
    $scope.addUser = function (size, current_user) {
        ...
        modalInstance.result.then(function (user) {
            if (!user._id) {
                CosmosService.post('/ios/service/cosmos.users/', user, function (data) {
                        $scope.getUsers();
                    },
                ...
                CosmosService.put('/ios/service/cosmos.users/' + user_id + '/', user, function (data) {
                        $scope.getUsers();
                    },
                    function (data, status) {
                        $scope.processError(data, status);
    ...
    $scope.removeUser = function (userIndex) {
        ...
        if (confirm('Are you sure you want to delete the user ' + user.username + '?')) {
            var user_id = user._id;
            CosmosService.delete('/ios/service/cosmos.users/' + user_id + '/', function (data) {
                    $scope.getUsers();
                },
                function (data, status) {
                    $scope.processError(data, status);
    ...
    $scope.getRoles = function () {
        CosmosService.get('/ios/service/cosmos.rbac.object.role/', function (data) {
                $scope.roles = data;
            },
            function (data, status) {
                $scope.processError(data, status);
    ...
    $scope.getUsers = function () {
        CosmosService.get('/ios/service/cosmos.users/', function (data) {
                $scope.users = data;
            },
            function (data, status) {
                $scope.processError(data, status);
